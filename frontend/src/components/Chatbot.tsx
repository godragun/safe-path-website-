  ...
  return (
    <>
      ...
      <button 
        ...
        className={`fixed bottom-6 right-6 p-4 bg-red-500 hover:bg-red-400 text-white rounded-full shadow-lg shadow-red-500/30 transition-all duration-300 z-50 ${isOpen ? "scale-0 opacity-0" : "scale-100 opacity-100 hover:scale-110"}`}
      >
      ...
      {/* Chat Window */}
      <div 
        className={`fixed bottom-6 right-6 w-80 sm:w-96 bg-[#0a0e1a] border border-slate-800 rounded-2xl shadow-2xl flex flex-col z-50 transition-all duration-300 transform origin-bottom-right ${isOpen ? "scale-100 opacity-100" : "scale-0 opacity-0 pointer-events-none"}`}
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-800 bg-slate-900/50 rounded-t-2xl">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-semibold text-slate-100 text-sm">SafePath AI</h3>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${msg.role === "user" ? "bg-slate-800" : "bg-red-500/10"}`}>
                {msg.role === "user" ? <User className="w-4 h-4 text-slate-300" /> : <Bot className="w-4 h-4 text-red-500" />}
              </div>
              <div className={`max-w-[75%] rounded-2xl px-4 py-2 text-sm ${
                msg.role === "user" 
                  ? "bg-slate-800 text-slate-100 rounded-tr-sm" 
                  : "bg-red-500/10 border border-red-500/20 text-slate-200 rounded-tl-sm"
              }`}>
                {msg.content}
              </div>
            </div>
        ...
        {/* Input */}
        <div className="p-3 bg-slate-900/50 border-t border-slate-800 rounded-b-2xl">
          <form onSubmit={handleSubmit} className="flex gap-2">
            <input
              ...
              className="flex-1 bg-slate-800/50 border border-slate-700 focus:border-red-500 rounded-xl px-4 py-2 text-sm text-slate-200 outline-none transition-colors"
            />
            <button 
              type="submit"
